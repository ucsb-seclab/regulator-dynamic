    ...
    while not stop:
        ...
        with global_lock:
            my_curr.execute(
                ...
                join regexps_fuzz_results_perffuzz rfr on rfr.regexp_id = r.id and rfr.length = 200
                join regexps_guess_pump_from_fuzz2_perffuzz rgf ON rgf.fuzz_result_id = rfr.id and rgf.pump_string is not null
                where rgf.id not in (select guess_pump_id from regexps_guess_pump_length_results3_perffuzz)
        ...
        exceeded_max = False
        try:
            witness: bytes = witness.tobytes()
            witness = asciify_witness(witness)
            assert pump_len <= len(witness)
            assert pump_pos + pump_len <= len(witness)
            if char_width == 2:
        ...
        with global_lock:
            my_curr.execute(
                """
                INSERT INTO regexps_guess_pump_length_results3_perffuzz (guess_pump_id, length_for_ten_s, num_pumps, fail_reason, exceeded_max_pumps)
                VALUES (%s,%s,%s,%s,%s)
                """,
