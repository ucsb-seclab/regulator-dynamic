...
string regexpPattern(Expr exp)
{
  (
    ...
    result = exp.(RegExpLiteral).getRoot().getRawValue()
  ) or
  (
    not (exp instanceof RegExpLiteral) and
...
from Expr exp
where
  // either this is already a regexp literal, or it is
  // coerced into one
  exp instanceof RegExpLiteral or
...
select
  ...
  , exp.getLocation().getFile() as file
