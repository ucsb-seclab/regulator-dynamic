...
inline int CompareCharsUnsigned2(const lchar* lhs, const rchar* rhs,
                                size_t chars, uint64_t &opcount) {
  // MOD NOTE: modified version of CompareCharsUnsigned which uses
  // `opcount` to count the number of comparisons performed
  const lchar* limit = lhs + chars;
  if (sizeof(*lhs) == sizeof(char) && sizeof(*rhs) == sizeof(char)) {
    // non-optimized memcmp
    while (lhs < limit) {
      opcount++;
      int r = static_cast<char>(*lhs) - static_cast<char>(*rhs);
      if (r != 0) return r;
      ++lhs;
      ++rhs;
    }
...
template <typename lchar, typename rchar>
inline int CompareChars2(const lchar* lhs, const rchar* rhs, size_t chars, uint64_t &opcount) {
  // MOD NOTE: modified version of CompareChars which uses
  // `opcount` to count the number of comparisons performed
  DCHECK_LE(sizeof(lchar), 2);
  DCHECK_LE(sizeof(rchar), 2);
  if (sizeof(lchar) == 1) {
    if (sizeof(rchar) == 1) {
      return CompareCharsUnsigned2(reinterpret_cast<const uint8_t*>(lhs),
                                  reinterpret_cast<const uint8_t*>(rhs), chars, opcount);
    } else {
      return CompareCharsUnsigned2(reinterpret_cast<const uint8_t*>(lhs),
    ...
    if (sizeof(rchar) == 1) {
      return CompareCharsUnsigned2(reinterpret_cast<const uint16_t*>(lhs),
                                  reinterpret_cast<const uint8_t*>(rhs), chars, opcount);
    } else {
      return CompareCharsUnsigned2(reinterpret_cast<const uint16_t*>(lhs),
                                  reinterpret_cast<const uint16_t*>(rhs),
