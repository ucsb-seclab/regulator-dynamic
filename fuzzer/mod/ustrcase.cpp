...
static int32_t _cmpFold(
    ...
    return _cmpFold(s1, length1, s2, length2, options, matchLen1, matchLen2, pErrorCode, _fake);
    // ------- (end) mod_mcl_2020 -------
}
...
// ------- mod_mcl_2020 -------
U_CFUNC int32_t
u_strcmpFold2(const UChar *s1, int32_t length1,
             const UChar *s2, int32_t length2,
             uint32_t options,
             UErrorCode *pErrorCode,
             uint64_t &opcount) {
    return _cmpFold(s1, length1, s2, length2, options, NULL, NULL, pErrorCode, opcount);
}
// ------- mod_mcl_2020 -------
